...
//	 Vector mappingsIn;
	 ...
	 public QueryInPdms(int bindport, InetSocketAddress bootaddress,
...
//			Generate the NodeIds Randomly
		 ...
		 for (int i = 0; i < glavsIn.size(); i++){
			 if (((GLAVMapping)glavsIn.elementAt(i)).getSchema().equals(peerName)){
				 System.out.println("GLAV " + peerName + ": ");
				 System.out.println(((GLAVMapping)glavsIn.elementAt(i)).printString());
				 app.setGLAVMapping((GLAVMapping)glavsIn.elementAt(i));
				 break;
			 }
		 }
	     ...
	     if (nodeSeq != 0){ //not the starting node
			 ...
			 FileReader queryfile = new FileReader("log/Query/Query.log");
			 BufferedReader fileInput = new BufferedReader(queryfile);
			 String query = fileInput.readLine();
			 app.setQuery(new Query(query));
			 app.sendQueryMsg(app.getNode().getLocalNodeHandle());
...
//	      int totalpeernum = Integer.parseInt(args[1]);
	    ...
	    // launch our node!
	    QueryInPdms driver = new QueryInPdms(bindport, bootaddress, 
