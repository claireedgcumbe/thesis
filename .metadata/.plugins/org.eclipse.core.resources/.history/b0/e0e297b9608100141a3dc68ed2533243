...
//					System.out.println("m_new_rel: " + m_new_rel.printString());
	...
	public void computeGLAVMapping(Predicate m_new_rel, MappingTable newMT){
		...
		for (int i = 1; i < newMT.rowNum; i++){	
			...
			cur_rel = (String)newMT.relStrings.elementAt(i);
            ...
            //need to create a new glav
				...
				for (int k = 0; k < attributes.size();k++){
					System.out.print("    "+((IPValue)attributes.elementAt(k)).printString());
...
//					System.out.println("duplicate");
				...
				query_head.setFunctionHead(((GLAVMapping) 
						m_glav.elementAt(schemaI)).getCurrentFreeName());	 // this is just the name of the schema
...
//		Predicate a_rel = new Predicate();
	...
	public static void testSchemaMerge(){
		...
		SchemaMediation s_merge = new SchemaMediation();
