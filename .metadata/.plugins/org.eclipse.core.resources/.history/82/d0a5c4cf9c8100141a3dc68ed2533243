...
public class Statement implements Serializable{
    //note, this has the correct version of answersToQuery
    ...
    protected ArrayList<String> express;
	
    public ArrayList<String> getExpress() {
		return express;
	}

	public void setExpress(ArrayList<String> express) {
	...
	public void setExpressByIndex(String exp, int index) {
		this.express.set(index, exp);
	}
...
//    public boolean setSubgoal(Predicate[] aval, int index) {
//        if(index>=_num_subgoals||index<0)
//        	return false;
//        _num_subgoals = index;
//        int num_total = _num_subgoals+aval.length-1;
//        if (num_total > _max_size){
//            //then we can add without a problem, otherwise we need to 
//            //copy things over.
//            //I realize that this is much like doing a vector, but this
//            //may allow us to be more efficient.
...
//            _max_size = _max_size * 2;
//        }
//        int right = _num_subgoals - index -1;
...
//   }
    ...
    public boolean variableIsDistinguished(IPValue avar){
    ...
    public Vector predicatesThatContainVariable(IPValue avar){
    ...
    public boolean containsPredicateName(String p_name)
    {
    	int counter;
    	for (counter = 0; counter < size(); counter++)
    	{
    		if (subgoalI(counter).getFunctionHead().equals(p_name))
    		{
    			return true;
    		}
    	}
    ...
    public StringBuffer printString(){
	   if (head == null)
        ...
        if(express.size()>0&&express!=null) {
        	for(int i=0; i<express.size(); i++)
        		retval.append(","+express.get(i));
        }
	   ...
	   Mapping a_mapping;
                ...
                !tuples.isDone();tuples.next()){
                    ...
                    if (atuple.function.equals(apred.function)){
                        ...
                        for (invalid = false,apred.first(); invalid == false && !apred.isDone();apred.next()){
                            ...
                            for(j = 0, flag = false; 
                            flag == false && j < newAssign.size();j++){
                                current_mapping = (Mapping)newAssign.elementAt(j);
        ...
        for (i = 0; i < assignments.size();i++){
            ...
            j++){
			 ...
			 //also we want to make sure that we didn't map a constant to
			 //a non-constant and that it was the same constant.
			 a_mapping = (Mapping)an_assignment.elementAt(j);
                if (a_mapping.mapping.equals(" ") ||
				(a_mapping.variable.isAnyConstant() && !a_mapping.variable.equals(a_mapping.mapping))){
                    invalid = true;
                }
            }
            if (!invalid){
    ...
    public boolean read(String values){
	   ...
	   //need to check and make sure that there wasn't extra crap hanging off the edge
	   if (!substr.trim().equals("")){
		   System.out.println("substr"+substr);
		   if(substr.contains("=")) {
			   addExpress(substr);
			   System.out.println("express"+express.get(0));
		   } else 
			   System.out.println("Improperly formed Predicate " + values + " in Statement.read()");
	   }
    ...
    public boolean isProjectionOnly()
    {
    	//needed for creating mediated schema in semantic merge. 
    	//note, assumes that there are no constants
    	int num_vars = this.findUniqueVariables().size(); 
    	if (_num_subgoals != 1)
    	{
    		return false;
    	}
    	if (this.getDistinguishedVariables().size() == num_vars)
    	{
    		return false;
    	}
    	if (this.subgoalI(0).size() != num_vars)
    	{
    		return false;
    	}
    	return true;
    }//end isProjectionOnly
    
    public static void testIsProjectionOnly(){
    	Statement bob = new Statement("q(x):-e(x)");
    	System.out.println("for " + bob.printString() + " isProjectionOnly = " + bob.isProjectionOnly());
    	bob = new Statement("q(x):-e(x,x)");
    	System.out.println("for " + bob.printString() + " isProjectionOnly = " + bob.isProjectionOnly());
    	bob = new Statement("q(x):-e(x,y),e2(x,z)");
    	System.out.println("for " + bob.printString() + " isProjectionOnly = " + bob.isProjectionOnly());
    	bob = new Statement("q(x):-e(x,y)");
    	System.out.println("for " + bob.printString() + " isProjectionOnly = " + bob.isProjectionOnly());
    ...
    public static void main (String args[])
    {
       ...
       Vector returned;
       q.read("q(b):-e1(a),e2(b)");
       v.read("q(y):-e1('5'),e2(y)");
       q2.read("q(r,s):-e1(r),e1(s),e1(t),e2(r),e2(s),e2(t),e1(t),e4(t)");
       q3.read("q(r,s):-e1(r),e1(s),e1(t),e2(r),e2(s),e3(t)");
	  System.out.println("q = " + q.printString());
	  System.out.println("v = " + v.printString());
	  if (q.contains(v)){
		 System.out.println("q contains v");
	  }
	  else{
		 System.out.println("q does not contain v");
	  }
	  if (v.contains(q)){
		 System.out.println("v contains q");
	  }
	  else{
		 System.out.println("v does not contain q");
	  }
	  */
