...
public class NoExistentialCheckAlgorithm extends NewAlgorithm{
	//private Vector MDList;
	public NoExistentialCheckAlgorithm(){
		super();
	...
	public Vector combineMDList(){
		...
		MetaMDWithoutExistentialCheck current_md;
		MDWithoutExistentialCheck a_md;
		current.addElement(new MetaMDWithoutExistentialCheck(myQuery.size()));
		for (i = 0; i < myQuery.size(); i++){
			...
			for (j = 0; j < current.size();j ++){
				...
				current_md = (MetaMDWithoutExistentialCheck)current.elementAt(j);
				if (!current_md.checkCovered(i)){
					for (k = 0; k < MDList.size();k++){
						//loop over the different choices in our MD's
						a_md = (MDWithoutExistentialCheck)MDList.elementAt(k);
						if (a_md.checkCovered(i)){
							MetaMDWithoutExistentialCheck new_md = current_md.copy();
